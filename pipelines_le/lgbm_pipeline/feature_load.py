...
def load_data(path_pattern='../../training_set?/*.psv', max_files=None) -> list[pl.DataFrame]:
	...
	for file_path in tqdm(psv_files, desc='Loading files'):
		...
		try:
			...
			patient = patient.drop(['Unit1', 'Unit2', 'HospAdmTime', 'ICULOS'])
			patient = patient.cast({pl.UInt8: pl.Boolean})
			patients.append(patient)
		except TypeError as err:
			raise TypeError(f"{err} occured parsing {patient_id}")
			#patient.to_parquet(parquet_path)

		#multi = pl.MultiIndex.from_product([[patient_id], pl.to_timedelta(patient.index, 'h')], names=['PatientId', 'Time'])
