...
def create_window_feature_vector(patient_df: pd.DataFrame, window=6):
    ...
    for each row, but only using the specified VITALS_COLUMNS.
    Returns (X_rolled, y_aligned) for that patient.
    """
    y = patient_df["SepsisLabel"]
    
    X_vitals = patient_df.drop(columns=["SepsisLabel"])
    ...
    # Concatenate all rolling features along columns
...
# 1. Load the data (dictionary of {filename -> DataFrame})
